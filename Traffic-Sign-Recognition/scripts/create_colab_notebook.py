 ...
 "cells": [
  ...
  },
  {
   ...
   "source": [
    ...
    "\n",
    "# Set path to raw GTSDB dataset folder (containing Train/ and Test/ subfolders)\n",
    "RAW_DATA_DIR = '/content/GTSDB_Train_and_Test'\n",
    "\n",
    "# Extract zip automatically if present\n",
    "if not os.path.exists(RAW_DATA_DIR) and os.path.exists('/content/GTSDB_Train_and_Test.zip'):\n",
    "    print('Extracting GTSDB_Train_and_Test.zip...')\n",
    "    !unzip -q /content/GTSDB_Train_and_Test.zip -d /content/GTSDB_Train_and_Test\n",
    "\n",
    "print('Checking dataset structure:')\n",
    "if os.path.exists(RAW_DATA_DIR):\n",
    "    print('Train images count:', len(glob.glob(os.path.join(RAW_DATA_DIR, 'Train', 'images', '*.*'))))\n",
    "    print('Test images count:', len(glob.glob(os.path.join(RAW_DATA_DIR, 'Test', 'images', '*.*'))))\n",
    "else:\n",
    "    print('⚠️ RAW_DATA_DIR not found yet. Please upload GTSDB_Train_and_Test folder or zip to Colab!')"
   ]
  },
  {
   "cell_type": "markdown",
  ...
  },
  {
   ...
   "source": [
    ...
    "print('Copying held-out test images...')\n",
    "copy_and_clean(glob.glob(os.path.join(test_img_dir, '*.*')), target_dirs['test_img'], target_dirs['test_lbl'], test_lbl_dir)\n",
    "\n",
    "# Create dataset.yaml\n",
    "yaml_path = os.path.join(YOLO_DATA_DIR, 'dataset.yaml')\n",
    "yaml_content = f'path: {YOLO_DATA_DIR}\\ntrain: images/train\\nval: images/val\\ntest: images/test\\n\\nnames:\\n'\n",
